                ...
                <div class="card-body">
                    <form action="{{ route('service.update',['id' => $service->id]) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="service_name" value="{{ $service->service_name }}" class="form-control" id="name" placeholder="Service title">
                        </div>
                        
                        <div class="form-group">
                            <label for="Short description">About</label>
                            <textarea class="form-control" name="short_description"  id="message" rows="3" placeholder="Description">{{ $service->short_description }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="image">Service Image</label>
                            <input type="file"  name="service_image" class="form-control" id="email" placeholder="Enter Image">
                            <img src="{{asset('storage/service/'.$service->service_image) }}" height="200px" width ="200px"alt="">
                        </div>
